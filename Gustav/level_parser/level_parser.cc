...
std::vector<Slow_Bird*> Level_Parser::get_slow_birds() const
{
    auto xml_blocks { get_objects_as_xml("birds") };
    std::vector<Slow_Bird*> birds;
    for(const auto xml_block : xml_blocks)
    {
        ...
        delete xml_block;
        birds.push_back(new Slow_Bird {position, dimensions, name});
    }
