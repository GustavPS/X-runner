...
Movable_Object::Movable_Object(const sf::Vector2f& position,
    ...
    : Simulatable_Object { position, size, type, solid, texture }
{}
...
std::vector<Object*> Movable_Object::simulate(const int simulation_cycles,
                                              const std::vector<const Object*> &objects)
{
    ...
    if (m_distance.x != 0)
    {
        ...
        steps = { m_distance.x / simulation_cycles, 0.f };
    ...
    if (m_distance.y != 0)
    {
        ...
        steps = { 0.f, m_distance.y / simulation_cycles };
        m_shape.setPosition(m_shape.getPosition() + steps);
        for (auto *colliding_object : get_colliding_objects(objects))
            handle_moving_collision(colliding_object, steps);
    }
