...
int main()
{
    ...
    {
        if (object->name == "top")
            blocks.push_back( new Block {object->position, object->dimensions, "jump"} );
    ...
    {
        if (object->name == "jump")
            std::cerr << "x: " << object->position.x << "   y: " << object->position.y << "\n"
                      << "w: " << object->dimensions.x << "   h: " << object->dimensions.y << "\n";
    ...
    while(window.isOpen())
    {
        ...
        {
            player.move(steps);
            if (colliders = player.collided()) {
                collision_handler(player, colliders);
            }
