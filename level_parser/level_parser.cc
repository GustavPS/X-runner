    ...
    const std::unordered_map<std::string, sf::Texture*> &textures) const
{
         ...
         ; xml_objectgroup = xml_objectgroup.next_sibling("objectgroup"))
    {
             ...
             ; xml_object = xml_object.next_sibling("object"))
        {
            ...
            if (type == "player")
            {
                xml_objects.push_back( new Player { position, size, type, textures.find(type)->second, speed } );
            }
            else if (type == "slow_bird")
            {
                xml_objects.push_back( new Slow_Bird { position, size, type, textures.find(type)->second, speed, direction } );
            }
            else if (type == "boost_bird")
            {
                xml_objects.push_back( new Boost_Bird { position, size, type, textures.find(type)->second, speed, direction } );
            }
            else if (type == "bomb_bird")
            {
                xml_objects.push_back( new Bomb_Bird { position, size, type, textures.find(type)->second, speed, direction, textures.find("nfbb")->second } );
            }
            else if (type == "nfbb")
            {
                xml_objects.push_back( new NFBB { position, size, type, textures.find(type)->second } );
            }
        }
    }
