                             ...
                             const float gravity_constant)
{
    ...
    {
        if (m_boost_bird_clock.getElapsedTime().asSeconds() < 2.25f)
        {
    ...
    if (m_nfbb_debuffs.size())
    {
        if (m_nfbb_clock.getElapsedTime().asSeconds() < 2.f)
        {
            speed_modifier *= std::pow(0.66f, m_nfbb_debuffs.size());
            //boost_bird_buffs.clear();
        }
        else
        {
            m_nfbb_debuffs.clear();
        }
    }
                              ...
                              const sf::Vector2f &steps)
{
    ...
    if (object->is_solid())
    {
        if (steps.y > 0.f)
        {
            ...
            else if (type == "goal" || type == "play" || type == "quit")
            {
                m_oog_action = type;
            }
        }
...
void Player::handle_static_collision(const Object *object)
{
    ...
    {
        if (m_nfbb_debuffs.find(object)
            == m_nfbb_debuffs.end())
        {
            m_nfbb_clock.restart();
            m_nfbb_debuffs.insert(object);
        }
    }
}
...
const std::string& Player::get_oog_action() const
    { return m_oog_action; }